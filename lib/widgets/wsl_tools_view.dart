...
class WSLToolsView extends StatelessWidget {
  final List<WSLTool> wslTools;
  final VoidCallback onInstallAll;
  final Function(WSLTool) onInstall;
  final Function(WSLTool) onCheck;
  final Function(WSLTool) onTest;
  final Function(WSLTool) onRemove;
  final Function(WSLTool) onReinstall;
  final Function(WSLTool) onUpdate;
                  ...
                  builder: (context, constraints) {
                    ...
                    return Container(
                      ...
                      child: ListView.builder(
                        itemCount: wslTools.length,
                        itemBuilder: (context, index) {
                          final tool = wslTools[index];
                          return Container(
                            margin: EdgeInsets.only(bottom: isMobile ? 8 : 12),
                            child: _buildWSLToolCard(context, tool, isMobile),
                          );
                        },
                      ),
                    );
  ...
  Widget _buildWSLToolCard(BuildContext context, WSLTool tool, bool isMobile) {
        ...
        child: IntrinsicHeight(
          child: Row(
            ...
            children: [
              // Icon
              Icon(
                IconHelper.getWSLToolIcon(tool.icon ?? 'terminal'),
                size: isMobile ? 32 : 40,
                color: color,
              ),
